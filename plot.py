...
class MiaAnalysisPlotter:
    ...
    def _load_exp2_data(self):
        """加载实验2 MIA的数据"""
        import pickle
        
        # 从配置获取实验参数
        if 'EXPERIMENT_2_MIA' not in EXPERIMENT_CONFIGS:
            print("❌ 错误: 找不到 EXPERIMENT_2_MIA 配置")
            return {}, [], []
        
        config = EXPERIMENT_CONFIGS['EXPERIMENT_2_MIA']
    ...
    def plot(self):
        ...
        dataset_mia_map, noise_list_exp2, datasets_exp2 = self._load_exp2_data()
        ...
        if not dataset_mia_map:
            print("❌ 错误: 没有找到实验2的任何数据")
            return
        
        # 确保两个实验使用相同的数据集和噪声列表
        datasets = list(set(datasets_exp1) & set(datasets_exp2))
        if not datasets:
