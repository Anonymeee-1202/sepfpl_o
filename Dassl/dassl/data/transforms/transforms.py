...
def build_transform(cfg, is_train=True, choices=None):
    ...
    if cfg.INPUT.NO_TRANSFORM:
        ...
        logger.info("Note: no transform is applied!")
        return None
...
def _build_transform_train(cfg, choices, target_size, normalize):
    logger = require_global_logger()
    logger.info("Building transform_train")
    tfm_train = []
    ...
    if all(conditions):
        logger.info(f"+ resize to {target_size}")
    ...
    if "random_resized_crop" in choices:
        ...
        logger.info(f"+ random resized crop (size={input_size}, scale={s_})")
        tfm_train += [
    ...
    if "randomgrayscale" in choices:
        logger.info("+ random gray scale")
        tfm_train += [RandomGrayscale(p=cfg.INPUT.RGS_P)]

    if "gaussian_blur" in choices:
        logger.info(f"+ gaussian blur (kernel={cfg.INPUT.GB_K})")
    ...
    if "cutout" in choices:
        ...
        logger.info(f"+ cutout (n_holes={cutout_n}, length={cutout_len})")
    ...
    if "instance_norm" in choices:
        logger.info("+ instance normalization")
...
def _build_transform_test(cfg, choices, target_size, normalize):
    logger = require_global_logger()
    logger.info("Building transform_test")
    tfm_test = []

    interp_mode = INTERPOLATION_MODES[cfg.INPUT.INTERPOLATION]
    input_size = cfg.INPUT.SIZE

    logger.info(f"+ resize the smaller edge to {max(input_size)}")
    tfm_test += [Resize(max(input_size), interpolation=interp_mode)]

    logger.info(f"+ {target_size} center crop")
